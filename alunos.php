<?php
/**
 * Projeto Escola - Cadastro de Alunos
 */
...
if (isset($_REQUEST["act"]) && $_REQUEST["act"] == "save" && $nome != "") {
    try {
        if ($id != "") {
            $stmt = $conexao->prepare("UPDATE alunos SET nome=?, email=?, celular=? WHERE id = ?");
            $stmt->bindParam(4, $id);
        } else {
            $stmt = $conexao->prepare("INSERT INTO alunos (nome, email, celular) VALUES (?, ?, ?)");
        }
